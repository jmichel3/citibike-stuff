...
class Model():
    def __init__(self, args, infer=False):
        ...
        output_w = tf.Variable(tf.zeros([args.rnn_size, NOUT], dtype=tf.float32))
        output_b = tf.Variable(tf.zeros([NOUT, 1], dtype=tf.float32))
        outputs = tf.Variable(tf.zeros([NOUT, 1], dtype=tf.float32))
        print("tf.shape(outputs)={}".format(tf.shape(outputs)))
        print("tf.shape(output_w)={}".format(tf.shape(output_w)))
        print("tf.shape(output_b)={}".format(tf.shape(output_b)))

        # inputs = tf.split(axis=1, num_or_size_splits=args.seq_length, value=self.input_data)
        # inputs = [tf.squeeze(input_, [1]) for input_ in inputs]
        # print("self.input_data.shape = {}\n".format(tf.shape(self.input_data)))
        ...
        # outputs = tfa.seq2seq.Decoder(inputs, zero_state)
        output = outputs

        # outputs = tf.Variable(initial_value=0.*NOUT, shape=tf.TensorShapeNOUT)
        # output = tf.reshape(tf.concat(axis=1, values=outputs), [-1, args.rnn_size])
        
        # output = tf.nn.xw_plus_b(inputs, output_w, output_b)
        output = tf.linalg.matvec(inputs, output_w)
        # output = tf.nn
